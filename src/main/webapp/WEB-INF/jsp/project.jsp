...
<div align="center" class="buttons">
	<button type="button" class="btn btn-primary btn-lg"
		data-toggle="modal" data-target="#myModal">Invest custom amount</button>
	<security:authorize
		access="#project.user.name != authentication.name && isAuthenticated()">
		<button type="button" class="btn btn-primary btn-lg"
			data-toggle="modal" data-target="#getreward">Invest particular amount, and get reward</button>
	</security:authorize>
	<button class="btn btn-success btn-lg" data-toggle="modal"
		data-target="#myModal1">Ask a question</button>
	<security:authorize access="#project.user.name == authentication.name">
		<button type="button" class="btn btn-primary btn-lg"
			data-toggle="modal" data-target="#RewardModal">Create reward</button>
	</security:authorize>
...
<form:form modelAttribute="questionandanswers"
	cssClass="form-horizontal projectForm">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Ask a question to
						the author of the project.</h4>
				</div>
				<div class="modal-body">

					<div class="form-group">
						<div class="col-sm-10">
							<form:textarea path="question" rows="5" cols="70"
	...
	<tbody>
		<c:forEach items="${qa}" var="qa">
			<tr>
				...
				<td><c:out value="${qa.user.name}" /></td>
				<td><c:out value="${qa.answer}" /> <security:authorize
						access="#project.user.name == authentication.name && #qa.answer == null">

						<button class="btn btn-success" data-toggle="modal"
							data-target="#myModal3">Answer the question</button>
...
<form:form action="addreward.html" modelAttribute="reward"
	cssClass="form-horizontal projectForm">
	<!-- Modal -->
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						<label for="name" class="col-sm-2 control-label">Reward
							description:</label>
						<div class="col-sm-10">
							<form:textarea path="description" rows="5" cols="70"
								cssClass="form-control" />
...
<table class="table table-bordered table-hover table-striped">
	...
	<c:forEach items="${rewardList}" var="rewardList">
		<tbody>
			<tr>
				<td><c:out value="$${rewardList.amount}" /></td>
				<td><c:out value="${rewardList.description}" /></td>
			</tr>
		</tbody>
	</c:forEach>
</table>

<form:form action="getreward.html" modelAttribute="userreward"
	cssClass="form-horizontal projectForm">
	<!-- Modal -->
	<div class="modal fade" id="getreward" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-body">
					<div class="col-sm-10">
						<form:select path="id">
							...
							<form:options items="${rewardList}" itemValue="id"
								itemLabel="amount" />
						</form:select>
						<form:hidden path="project.id" value="${project.id}" />
					</div>
